...
db.connect(function(err) {
    ...
    //getDog(microchip_ID)
    db.query("SELECT d.Microchip_ID, d.Dog_Name, d.sex, d.Age, size.Size_Name, d.Dog_Description, d.Favourite_Activity, d.Favourite_Treat, d.Special_Needs, b.Dog_Friendly, b.Cat_Friendly, b.Child_Friendly, b.Adult_Friendly, b.Shed, s.Shelter_Name, s.Contact_Person, s.Cellphone, s.Email, s.Location FROM Dog d INNER JOIN Size ON d.Size_ID = size.Size_ID INNER JOIN Behaviour b ON d.Microchip_ID = b.Microchip_ID INNER JOIN Shelter s ON d.Shelter_ID = s.Shelter_ID WHERE d.Microchip_ID = 123456789123456", function (err, result, fields) {
      if (err) throw err;
      console.log(result);
    });

    //getDogBreed
    db.query("SELECT d.Microchip_ID, d.Dog_Name, b.Breed_Name FROM Dog d INNER JOIN Dog_Breed db ON d.Microchip_ID = db.Microchip_ID INNER JOIN Breed b On db.Breed_ID = b.Breed_IDB WHERE d.Microchip_ID = 123456789123456", function (err, result, fields) {
      if (err) throw err;
      console.log(result);
    });
